...
- (void)viewDidLoad
{
    ...
    NSDictionary *restmp=[loadingapi getFollowerList:Uid];
    NSLog(@"aaa %@",restmp);
    ...
    for (int i=0; i<cnt; i++) 
    {
        ...
        for (NSDictionary *dic in result) 
        {
            ...
            [deletaButton addTarget:viewController1 action:@selector(modalButtonDidPush) forControlEvents:UIControlEventTouchUpInside];
            //deletaButton.center = CGPointMake(6, 51);
            //deletaButton.contentMode = UIViewContentModeScaleToFill;
            [self.view addSubview:deletaButton];
            
            //削除（固定文言）
            UILabel *deletaLabel = [[UILabel alloc] init];
            deletaLabel.frame = CGRectMake(106, 101 + margin * i, 200, 20);
            ...
            deletaLabel.text = @"削除";
            [self.view addSubview:deletaLabel];
            */
            //横線
